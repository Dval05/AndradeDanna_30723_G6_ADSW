...
import ec.edu.espe.estudiantecrud.logica.observer.AuditoriaObserver;
import ec.edu.espe.estudiantecrud.logica.observer.HistorialObserver;
import ec.edu.espe.estudiantecrud.logica.strategy.BuscarPorID;
import ec.edu.espe.estudiantecrud.logica.strategy.BuscarPorNombre;
import ec.edu.espe.estudiantecrud.logica.strategy.BuscarPorEdad;

public class FormularioCrudEstudiante extends JFrame {
    private final ControlEstudiante controlador;
    private JTextField txtId, txtNombre, txtEdad, txtBusqueda;
    ...
    private final Color colorStrategy = new Color(70, 130, 180);
    ...
    /**
     * Observer: Registra los observadores (Auditoría e Historial)
     * en el gestor de eventos del controlador.
     */
    private void registrarObservadores() {
        controlador.getGestorEventos().agregarObservador(new AuditoriaObserver());
        controlador.getGestorEventos().agregarObservador(new HistorialObserver());
    }

    private void configurarVentana() {
        setTitle("EstudianteCRUD - ESPE (Observer + Strategy)");
        setSize(650, 580);
    ...
    private void inicializarComponentes() {
        ...
        // ====== PANEL DE BÚSQUEDA (Strategy) ======
        JPanel pnlBusqueda = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 6));
        pnlBusqueda.setBackground(new Color(235, 242, 252));
        pnlBusqueda.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(1, 0, 1, 0, new Color(200, 210, 230)),
            BorderFactory.createEmptyBorder(4, 12, 4, 12)
        ));

        JLabel lblBusqueda = new JLabel("Buscar:");
        lblBusqueda.setFont(fuenteEtiqueta);
        lblBusqueda.setForeground(colorStrategy);

        txtBusqueda = new JTextField(15);
        txtBusqueda.setFont(fuenteEtiqueta);
        txtBusqueda.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(180, 200, 225)),
                BorderFactory.createEmptyBorder(4, 6, 4, 6)
        ));

        JLabel lblEstrategia = new JLabel("Estrategia:");
        lblEstrategia.setFont(fuenteEtiqueta);
        lblEstrategia.setForeground(colorStrategy);

        cmbEstrategia = new JComboBox<>(new String[]{"Buscar por ID", "Buscar por Nombre", "Buscar por Edad"});
        cmbEstrategia.setFont(fuenteEtiqueta);
        cmbEstrategia.addActionListener(e -> cambiarEstrategia());

        JButton btnBuscar = new JButton("Buscar");
        btnBuscar.setFont(fuenteBoton);
        btnBuscar.setForeground(Color.WHITE);
        btnBuscar.setBackground(colorStrategy);
        btnBuscar.setFocusPainted(false);
        btnBuscar.addActionListener(e -> clickBuscar());

        pnlBusqueda.add(lblBusqueda);
        pnlBusqueda.add(txtBusqueda);
        pnlBusqueda.add(lblEstrategia);
        pnlBusqueda.add(cmbEstrategia);
        pnlBusqueda.add(btnBuscar);

        // ====== PANEL CENTRAL (Búsqueda + Tabla) ======
        JPanel pnlCentralCompleto = new JPanel(new BorderLayout());
        pnlCentralCompleto.setBackground(colorFondo);
        pnlCentralCompleto.add(pnlBusqueda, BorderLayout.NORTH);
        ...
        pnlCentralCompleto.add(scroll, BorderLayout.CENTER);

        add(pnlCentralCompleto, BorderLayout.CENTER);
    }

    /**
     * Strategy: Cambia la estrategia de búsqueda según la selección del ComboBox.
     */
    private void cambiarEstrategia() {
        int seleccion = cmbEstrategia.getSelectedIndex();
        switch (seleccion) {
            case 0:
                controlador.getContextoBusqueda().setEstrategia(new BuscarPorID());
                break;
            case 1:
                controlador.getContextoBusqueda().setEstrategia(new BuscarPorNombre());
                break;
            case 2:
                controlador.getContextoBusqueda().setEstrategia(new BuscarPorEdad());
                break;
        }
    }

    /**
     * Strategy: Ejecuta la búsqueda con la estrategia activa.
     */
    private void clickBuscar() {
        String criterio = txtBusqueda.getText().trim();
        if (criterio.isEmpty()) {
            mostrarMensaje("Ingrese un criterio de búsqueda.");
            return;
        }
        List<Estudiante> resultados = controlador.buscarEstudiante(criterio);
        if (resultados.isEmpty()) {
            mostrarMensaje("No se encontraron estudiantes con el criterio: " + criterio);
        }
